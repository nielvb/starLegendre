...
% sine problem. This shows the structure of the coefficient matrix and the
...
%% A sine problem
nu=1;
f = @(t) -1i*nu*sin(nu*(t+1)); % given
phi = @(t) exp(-1i*(1-cos(nu*(t+1)))); % solution
...
semilogy(1:M,abs(xx-xdot),'ro')
hold on
semilogy(1:M,abs(xx-xdot_trunc),'g+')

semilogy([M-nbnonZeros,M-nbnonZeros],[10^0,10^-30],'k--','LineWidth',2)
semilogy([M-nbnonZeros_prac,M-nbnonZeros_prac],[10^0,10^-30],'k--','LineWidth',2)
legend hide
...
if length(cc)<length(coeffsSol)
    cc = [cc(:);zeros(length(coeffsSol)-length(cc),1)];
end
...
% legend hide
% if saving
%     matlab2tikz('figurehandle',fff,'filename',['figs/contour_DE_m=',num2str(m),'nu=',num2str(nu),'.tikz'],'height', '\figureheight','width','\figurewidth', 'floatFormat','%.2g','showInfo', false)
% end
